--[[ 
A pure Lua JSON encoder/decoder.
...
local function encode_pretty(obj, as_key, indent)
  indent = indent or 0
  local s = {}
  ...
  if kind == 'array' then
    ...
    for i, val in ipairs(obj) do
      ...
      s[#s + 1] = '\n' .. next_indent_str
      s[#s + 1] = encode_pretty(val, false, indent + 1)
    end
    if #obj > 0 then s[#s + 1] = '\n' .. indent_str end
    s[#s + 1] = ']'
  elseif kind == 'table' then
    ...
    for k, v in pairs(obj) do
      ...
      first = false
      s[#s + 1] = '\n' .. next_indent_str
      s[#s + 1] = encode_pretty(k, true, indent + 1)
      s[#s + 1] = ': '
      s[#s + 1] = encode_pretty(v, false, indent + 1)
    end
    if not first then s[#s + 1] = '\n' .. indent_str end
    s[#s + 1] = '}'
...
function json.encode(obj, as_key)
  return encode_pretty(obj, as_key, 0)
end
